...
export default function BlogPost() {
    ...
    const related = publishedPosts.filter(p => p.slug !== slug && (p.category === post?.category || p.tags.some(t => post?.tags.includes(t)))).slice(0, 3);
    ...
    return (
        <div className="min-h-screen bg-white">
            {/* Cover */}
            <div className={`bg-gradient-to-br ${post.coverGradient} h-64 sm:h-80 relative flex items-end`}>
                <div className="absolute inset-0 bg-black/20" />
                <div className="relative max-w-3xl mx-auto w-full px-4 pb-8">
                    <Link to="/blogs"
                        className="inline-flex items-center gap-1.5 text-white/80 text-sm hover:text-white transition-colors mb-4">
                        <ArrowLeft size={14} /> Back to Blog
                    </Link>
                    <span className="inline-block px-3 py-1 rounded-full bg-white/20 backdrop-blur text-white text-xs font-semibold mb-3">
                        {post.category}
                    </span>
                </div>
            </div>
            ...
            {/* Article */}
            <div className="max-w-3xl mx-auto px-4 -mt-2 pb-16">
                <motion.div
                ...
                >
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400 mb-4">
                        <span className="flex items-center gap-1"><Calendar size={11} /> {format(new Date(post.publishedAt), 'dd MMMM yyyy')}</span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            {related.map(rel => (
                                <Link key={rel.id} to={`/blogs/${rel.slug}`}
                                    ...
                                    <div className={`bg-gradient-to-br ${rel.coverGradient} h-28 flex items-end p-3`}>
                                        <span className="text-[10px] font-semibold text-white/90 bg-white/20 px-2 py-0.5 rounded-full">{rel.category}</span>
                                    </div>
                                    <div className="p-4">
                                        ...
                                        <p className="text-[10px] text-slate-400 flex items-center gap-1">
                                            <Clock size={9} /> {rel.readTime}m · {format(new Date(rel.publishedAt), 'dd MMM')}
                                        </p>
                                    </div>
                                </Link>
                            ))}
